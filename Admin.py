...
def add_user(c_user, c_book, c_user_book, mycon):
    ...
    c_user.execute("insert into User_Details(Username, U_Name, About, U_Type, "
                   "Date_of_Joining, U_Password) values('{}', '{}', '{}', '{}', curdate(), '{}')".format(Username, U_Name, About, U_Type, U_Pswd))
    mycon.commit()
...
def add_book(c_user, c_book, c_user_book, mycon):
    ...
    c_user.execute("insert into Books(Book_ID, Book_Title, Author_Name, Genre, "
                   "Number_of_Copies, Number_of_Available_Copies) values('{}', '{}', "
                   "'{}', '{}', {},{})".format(Bk_ID, Bk_Title, A_Name, Genre, N_o_C,
                                                 N_o_C))
    mycon.commit()
...
def update_book(c_user, c_book, c_user_book, mycon, data_book):
    ...
    if choice == 'd':
        ...
        if check:
            ...
            if N_o_C - decrease > 0:
                c_book.execute("update Books set Number_of_Copies = {} where Book_ID = {}".format(N_o_C - decrease, Bk_ID))
            else:
                print("Invalid decrease in number of copies. Number of copies available = ", N_o_C)
    elif choice == 'i':
        for i in data_book:
            if i[0] == Bk_ID:
                N_o_C = i[4]
        ...
        if check:
            ...
            c_book.execute("update Books set Number_of_Copies = {}, Number_of_Available_Copies = {} where Book_ID = '{"
                           "}'".format(N_o_C + increase, N_o_A_C + increase, Bk_ID))
    mycon.commit()
